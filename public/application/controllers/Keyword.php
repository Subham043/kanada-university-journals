...
class Keyword extends CI_Controller {
    ...
    **/
    ...
    public function lists()
	{
        ...
        $config['base_url'] = base_url('admin/keyword/list');
        $config['total_rows'] = $this->m_keyword->get_count();
        $config['per_page'] = 10;
        ...
        $data['data'] = $this->m_keyword->get_list($config["per_page"], $page);

        $this->load->view('pages/keyword/list.php', $data);
    }
	...
	public function create()
	{
        ...
        $this->form_validation->set_rules('name', 'Keyword Name', 'trim|required|min_length[3]|max_length[200]|regex_match[/^[a-z 0-9~%.:_\@\-\/\&+=,]+$/i]', array('regex_match' => 'Enter a valid %s'));
    ...
    public function edit($id)
	{
        ...
        if($data['data']->code==$this->input->post('code')){
            $this->form_validation->set_rules('code', 'Keyword Code', 'trim|required|min_length[3]|max_length[200]|regex_match[/^[a-z 0-9~%.:_\@\-\/\&+=,]+$/i]', array('regex_match' => 'Enter a valid %s', 'is_unique'=>'This keyword code is already in use'));
        }else{
            $this->form_validation->set_rules('code', 'Keyword Code', 'trim|required|min_length[3]|max_length[200]|is_unique[keyword.code]|regex_match[/^[a-z 0-9~%.:_\@\-\/\&+=,]+$/i]', array('regex_match' => 'Enter a valid %s', 'is_unique'=>'This keyword code is already in use'));
        }
    ...
    public function delete($id)
	{
        ...
        if ($this->m_keyword->delete($keyword_id)!=FALSE) {
