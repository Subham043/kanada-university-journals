                            ...
                            <div class="card">
                                <div class="card-header">
                                    <ul class="nav nav-tabs-custom rounded card-header-tabs border-bottom-0">
                                        ...
                                        <li class="nav-item">
                                            <a class="nav-link" href="<?php echo base_url('department/'.$this->encryption_url->safe_b64encode($data->id).'/book'); ?>">
                                                <i class="far fa-user"></i>
                                                Books (<?php echo $book_count; ?>)
                                            </a>
                                        </li>
                                        <li class="nav-item">
                                            <a class="nav-link active" href="<?php echo base_url('department/'.$this->encryption_url->safe_b64encode($data->id).'/conference-proceedings'); ?>">
                                                <i class="far fa-user"></i>
                                                ...
                                                <div class="team-list row grid-view-filter" id="team-member-list">
                                                    <ol class="mb-0 sub-menu ps-3 vstack gap-2 mb-2">
                                                        <?php foreach ($conference as $item) { ?>
                                                            <li class="mb-2"><?php echo $item->prefix; ?>. <?php echo $item->first_name; ?> <?php echo $item->last_name; ?>, <?php echo date('Y', strtotime($item->date)); ?>, <?php echo $item->title; ?>, <a href="<?php echo $item->link; ?>" target="_blank"><?php echo $item->conference; ?></a>, <?php echo $item->place; ?>, <?php echo $item->isbn; ?></li>
                                                        <?php } ?>
                                                    </ol>
                                                    <?php echo $links; ?>
                                                </div>
